...
printf("transmissor_ficheiro\n");
        ...
        // cycle through
        const int buf_size = MAX_PAYLOAD_SIZE-1;printf("Ola_transmisor\n");
        ...
        while (bytes_read > 0)
        {
            ...
            printf("bytes_read=%d;  payload= %d\n",bytes_read,MAX_PAYLOAD_SIZE);
            if(bytes_read < 0) {
                ...
                buffer[0] = 1;printf("transmissor_Ola1:%x;;%d\n",buffer,bytes_read);
                ...
                // stop receiver
                buffer[0] = 0;printf("transmissor_Ola2\n");
                llwrite(buffer, 1);
    ...
    else
    {
        ...
        while (bytes_read >= 0)
        {
            bytes_read = llread(buffer);printf("recetor_ficheiro:%d\n",bytes_read);
            if(bytes_read < 0) {
                fprintf(stderr, "Error receiving from link layer\n");
                break;
            }
            else if (bytes_read > 0) {printf("recetor_Olá0: %x\n",buffer);
                if (buffer[0] == 1) {printf("recetor_Olá1\n");
                ...
                else if (buffer[0] == 0) {printf("recetor_Olá2\n");
                    printf("App layer: done receiving file\n");
                    break;
                }printf("recetor_Olá3\n");
            }
        }
printf("oppen e close direto\n");
        llclose(ll,1);
        close(file_desc);
        return 0;
    }
