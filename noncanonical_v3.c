...
int main(int argc, char** argv)
{
    ...
    char buf[255];
    unsigned char valor_lido;            //Valores lidos da porta serie
...
*/
    ...
    unsigned char packet[9999];     //defenir melhor o tamanho
...
*/    
        ...
        reject=0;
            ...
            while(STOP==FALSE){
                //////printf("valor lido_antes: %x\n",valor_lido);
                res = read(fd,&valor_lido,1);         //Lê um caratér e envia para a máquina de estados verificar para que estado avança
                //////printf("valor lido_depois: %x\n",valor_lido);
                ////////
                /*if (res < 0) {
                    perror("Erro na leitura");
                    break;
...
/*                    alarm(0);   */        //Para parar o alarme
            ...
            reject=0;
            while(STOP==FALSE){
                //printf("valor lido_antes: %x, %c\n",valor_lido);
                ret = read(fd, &valor_lido,1);
                //printf("valor lido_depois: %x, %c\n",valor_lido);
                if(valor_lido==0x5d){
                    //printf("valor lido_antes: %x, %c\n",valor_lido2);
                    ret = read(fd,&valor_lido2,1);
                    //printf("valor lido_depois: %x, %c\n",valor_lido2);
                ...
                BCC2_anterior=BCC2;
                BCC2=BCC2^packet[i];
                //printf("anterior %x : atual %x : enviado %x\npacote %s\n",BCC2_anterior,BCC2,BCC2_enviado, packet);
                i++;
            }
